 ...
 *
 * @see \Drupal\token_engine\ActorContext
 * @see \Drupal\token_engine\TokenResolutionEngine
 */
#[CoversClass(\Drupal\token_engine\ActorContext::class)]
#[Group('Token')]
...
class TokenTieredEnforcementTest extends TokenReplaceKernelTestBase {
  ...
  public function testViewerlessReplacementMatchesLegacyBridgeAndDeprecates(): void {
    $unenforcedBefore = $this->container->get('token_engine.unenforced_monitor')->count();
    $this->setUpCurrentUser(values: ['mail' => 'berdir@example.com'], permissions: []);

    $bubbleable_metadata = new BubbleableMetadata();
    $legacy = $this->container->get('token_engine.legacy_bridge')->generate(
      'current-user',
    ...
    $this->assertGreaterThan($unenforcedBefore, $this->container->get('token_engine.unenforced_monitor')->count(), 'Viewer-less resolution was recorded for the status report.');
  }
