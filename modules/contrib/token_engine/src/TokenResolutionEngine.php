...
use Drupal\token_engine\Discovery\AttributedTokenDiscovery;
...
final class TokenResolutionEngine implements TokenResolutionEngineInterface {
   ...
   * could be made configurable per-site (e.g. via token_engine.settings config) rather
   * than a constant.
   *
   * @var int
   */
  ...
  /**
   * Whether viewer-less resolution has been recorded for this engine instance.
   *
   * The engine is a request-scoped service, so instance-level once-guarding
   * amortizes the monitor call across a request no matter how many
   ...
   * replacement benchmark's scenario 4a). One record per engine instance
   * also gives the status report a meaningful unit: requests that resolved
   * without a viewer, not raw recursive generate() calls.
   */
  private bool $unenforcedRecorded = FALSE;

  /**
   * @param \Drupal\token_engine\LegacyTokenBridge $legacyBridge
   *   The legacy hook pipeline bridge.
   * @param \Drupal\token_engine\TokenRegistryInterface $registry
   *   The attributed-resolver registry.
   * @param \Drupal\token_engine\Discovery\AttributedTokenDiscovery $resolverLocator
   *   Used to retrieve live resolver instances by class name.
   * @param \Drupal\token_engine\TokenRendererInterface $renderer
   *   Serializes the terminal resolved value for the output context.
   * @param \Drupal\token_engine\ListDeltaResolver $listDeltaResolver
   ...
   * @param \Drupal\token_engine\UnenforcedUsageMonitor $unenforcedMonitor
   *   Records viewer-less resolution for the status report.
   */
  public function __construct(
    ...
    private readonly UnenforcedUsageMonitor $unenforcedMonitor,
  ) {}
   ...
   * @param \Drupal\token_engine\ChainPrefixMemo|null $memo
   ...
   * the chain, for BC with callers that predate the actor model. A contrib
   * module has no deprecation-and-removal lever, so this tier is a supported
   * mode whose use is made visible instead: recorded once per engine
   * instance on the status report, with a throttled log entry pointing
   * callers at options['viewer'].
   *
   * @param array<string, mixed> $options
   *   The replacement options.
   *
   * @return \Drupal\token_engine\ActorContext
  ...
  private function resolveActor(array $options): ActorContext {
    ...
    if (!$this->unenforcedRecorded) {
      $this->unenforcedRecorded = TRUE;
      $this->unenforcedMonitor->record();
    }
   ...
   *   The replacement options.
   *
   * @return \Drupal\token_engine\OutputContext
   ...
   *
   * @param \Drupal\token_engine\TokenResult $result
   ...
   *
   * @param \Drupal\token_engine\TokenDefinition|null $definition
