...
class ProcessDebugger {
    ...
    foreach ($history as &$entry) {
      ...
      if (array_key_exists('data', $entry)) {
        if ($entry['data'] === self::TOKEN_DATA_PREV) {
          ...
          // Expand entity reference markers within the token data.
  ...
  private static function expandRefs(array &$data): void {
    foreach ($data as &$value) {
      if (isset($value[self::TOKEN_DATA_REF]) && isset($data[$value[self::TOKEN_DATA_REF]]['data'])) {
        $value['data'] = $data[$value[self::TOKEN_DATA_REF]]['data'];
